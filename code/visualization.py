		...
		if sbrn_plt == False:
			...
			if show_samples == True:
				...
				if self.n_components == 3:
					...
					fig.savefig('<directory_path>', transparent=True, format='pdf')
				...
				plt.tight_layout()
				plt.show()
				fig.savefig('<directory_path>', bbox_inches='tight', transparent=True, format='pdf')
			...
			sns_plot.savefig('<directory_path>')
...
class GephiNetworks:
	...
	def plot(self, feat_range, random_sampling, corpus_size):
		...
		fob_nodes = open('<directory_path>', "w")
		fob_edges = open('<directory_path>', "w")
...
class RollingDelta:
	...
	def plot(self):
		...
		sns.plt.show()
		fig.savefig('<directory_path>', bbox_inches='tight')
