...
step_size = 100
test_dict = {}
function_words_only = open('<directory_path>').read().split()
invalid_words = []
rnd_dct = {'n_samples': 140,
		   'smooth_train': True,
		   'smooth_test': False}

results_file = open('<directory_path>', 'a')
	...
	for n_feats in feat_n_loop:
		...
		for val_1, val_2 in randomization_params:
			...
			for cv in cv_types:
				...
				model_location = '<directory_path>'.format(feat_type, str(n_feats), sampling_type, cv_name, str(best_c_param))
...
# for model in glob.glob('<directory_path>'):
# 	# Load model
...
# 	# test_set_location = '<directory_path>'
...
# df_2.to_excel(excel_writer='<directory_path>')
...
# 	fig.savefig('<directory_path>'.format(labels[label]), \
